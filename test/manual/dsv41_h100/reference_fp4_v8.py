...
def _fp4_index_logits_kernel(
    ...
    n_lim = tl.minimum(n_vis, L)
    if SKIP_INVALID:
        # Whole tile lies beyond the request's visible compressed prefix. The
        # reference path masks every lane of such a tile, so its only observable
        # effect is the -inf store; emit that and skip the slot load, payload and
        # scale loads, both dots and the head reduction. Only fully-invisible
        # tiles take this exit, so fully and partially visible tiles keep the
        # original arithmetic bit-for-bit. The store mask keeps the same
        # capacity/address contract: `out` is freshly allocated per call, so the
        # whole visible width must still be written.
        if lb * BLOCK_L >= n_lim:
            tl.store(
                out_ptr + b * L + offs_l,
...
def fp4_index_logits_decode(
    ...
    ``skip_invalid`` selects the whole-tile fast path (see the kernel); ``None``
    reads the ``SGLANG_OPT_DSV41_INDEXER_SKIP_INVALID_TILES`` switch. Either way
    the returned logits are identical for every device-visible length.
    """
    ...
    _fp4_index_logits_kernel[grid](
        ...
        BLOCK_L=BLOCK_L,
        SKIP_INVALID=skip_invalid if skip_invalid is not None else _skip_invalid_default(),
        num_warps=4,
    )
    return out


def _skip_invalid_default() -> bool:
    """Default of SGLANG_OPT_DSV41_INDEXER_SKIP_INVALID_TILES (unknown -> off)."""
    try:
        return bool(envs.SGLANG_OPT_DSV41_INDEXER_SKIP_INVALID_TILES.get())
    except Exception:
        return False
